...
export default function Calendar() {
  ...
  const calculateEventStyle = (startTime: string, endTime: string, eventsInSameTimeSlot = 0, eventIndex = 0) => {
    try {
      ...
      // Si hay eventos superpuestos, calcular el ancho y la posición horizontal
      let width = "100%"
      let left = "4px"
      let right = "4px"

      if (eventsInSameTimeSlot > 1) {
        const eventWidth = 100 / eventsInSameTimeSlot
        width = `calc(${eventWidth}% - 8px)`
        const leftPosition = eventIndex * eventWidth
        left = `calc(${leftPosition}% + 4px)`
        right = "auto"
      }

      return { top: `${top}px`, height: `${height}px`, width, left, right }
    } catch (error) {
      console.error("Error al calcular estilo del evento:", error)
      return { top: "0px", height: "80px", width: "100%", left: "4px", right: "4px" }
    }
  }
    ...
    <div className="relative min-h-screen w-full overflow-hidden">
      ...
      <main className="relative h-screen w-full pt-20 flex">
        <Sidebar
          ...
          events={events} // Pasar los eventos al componente Sidebar
        />
                  ...
                  <div className="flex-1 overflow-y-auto">
                    <div className="relative">
                      ...
                      {(() => {
                        ...
                        sortedEvents.forEach((event) => {
                          ...
                          const compatibleGroupIndex = groups.findIndex((group) => {
                            return group.every((groupEvent) => {
                              ...
                              return event.endTime <= groupEvent.startTime || event.startTime >= groupEvent.endTime
                            })
                          })
                        ...
                        // Asignar información de posicionamiento a cada evento
                        const organizedEvents = sortedEvents.map((event) => {
                          ...
                          // Encontrar la posición del evento dentro de su grupo
                          const eventsInSameTimeSlot = groups.length
                          const eventPositionInGroup = groupIndex

                          return {
                            ...event,
                            eventsInSameTimeSlot,
                            eventPositionInGroup,
                          }
                        })

                        return organizedEvents.map((event: any, i: number) => {
                          const eventStyle = calculateEventStyle(
                            event.startTime,
                            event.endTime,
                            event.eventsInSameTimeSlot,
                            event.eventPositionInGroup,
                          )
                          return (
                            <div
                              key={i}
                ...
                <>
                  ...
                  <div className="grid grid-cols-8">
                    ...
                    {weekDays.map((day, dayIndex) => {
                      // Agrupar eventos por franjas horarias superpuestas
                      const dayEvents = visibleEvents.filter((event: any) => isSameDay(event.date, day))
                      ...
                      return (
                        <div key={dayIndex} className="border-l border-white/20 relative">
                          ...
                          {organizedEvents.map((event: any, i: number) => {
                            const eventStyle = calculateEventStyle(
                              event.startTime,
                              event.endTime,
                              event.eventsInSameTimeSlot,
                    ...
                    {monthDays.map((day, i) => (
                      ...
                      >
                        ...
                        <div className="mt-1 space-y-1">
                          ...
                          {visibleEvents.filter((event: { date: Date }) => isSameDay(event.date, day)).length > 3 && (
